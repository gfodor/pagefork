...
;(function(global) {
  ...
  HTMLtoJSX.prototype = {
    ...
    _cleanInput: function(html) {
      ...
      html = html.replace(/<script(.|\s)*<\/script>/gim, '');

      // Noscript tags don't build children DOM elements either.
      html = html.replace(/<noscript(.|\s)*<\/noscript>/gim, '');
      ...
      switch (attribute.name) {
        ...
        default:
          ...
          if (isNumeric(attribute.value)) {
            ...
            result += '"' + attribute.value.replace('"', '&quot;').replace('&', '&amp;') + '"';
          }
          return result;
      }
    },
